...
const PostPage: React.FC = () => {
  ...
  return (
    <div className="max-w-screen-xl flex-1 mx-auto flex pt-8 w-full">
      {postQuery.isLoading && <Spinner className="flex justify-center items-center flex-1 w-full pb-8" />}
      {postQuery.isSuccess &&
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          ...
          <p className="pt-4 text-slate-500">
            {postQuery.data.text.length > 200 ? postQuery.data.text.slice(0, 200) : postQuery.data.text}
          </p>
          <img
            className="pt-6 max-h-[500px] max-w-full object-cover object-center"
            src={postQuery.data.image === null ? defaultImage : `https://megalab.pythonanywhere.com/${postQuery.data.image}`}
            alt="post title"
          />
          <p className="pt-4 pb-6 text-slate-500">
            {postQuery.data.text.length > 200
              ? postQuery.data.text.slice(200, postQuery.data.text.length)
              : null}
          </p>
          <IoShareSocialOutline size="24" color="#64748b" />
          <div className="pt-12">
            <p className="text-2xl font-medium pb-8">Комментарии</p>
            {postQuery.data.comment
              ? <CommentsList comments={postQuery.data.comment} postId={Number(postId)}/>
              : <p>No comments yet</p>
            }
          </div>
